...
11. [v2.1] _build_tg_msg(...) 以 Ellipsis 呼叫 → 補齊實際參數
12. [v2.1] 條件匹配迴圈 if _ok: 縮排錯誤 → 修正
13. [v2.1] 突破新高重複發送 Telegram → 合併 _tg_already_sent 與 _tg_on_bo 判斷
14. [v2.1] 跌破新低缺少 _tg_already_sent 去重 → 補上
"""
...
def compute_all_signals(data: pd.DataFrame, params: dict) -> pd.Series:
    results = []
...
with st.expander("⚡ 一鍵全部股票自動回測 & 更新 Telegram 條件表", expanded=False):
    ...
    _auto_wr_thr   = _bc1.number_input("合併勝率閾值 (%)", min_value=0, max_value=100, value=90, step=5, key="auto_bt_wr_thr")
    _auto_min_occ  = _bc2.number_input("最少出現次數", min_value=2, max_value=50, value=10, step=1, key="auto_bt_min_occ")
    _auto_pnl_thr  = _bc3.number_input("最低平均盈虧 (%)", min_value=-10.0, max_value=20.0, value=0.5, step=0.1, format="%.1f", key="auto_bt_pnl_thr")
    _auto_min_combo = _bc4.number_input("最少信號組合數", min_value=2, max_value=5, value=2, step=1, key="auto_bt_min_combo")
    _auto_max_combo = _bc5.number_input("最多信號組合數", min_value=2, max_value=5, value=3, step=1, key="auto_bt_max_combo")

    if _auto_interval in ("1m","5m","15m","30m","1h"):
        st.warning(f"⚠️ {_auto_interval} 短週期：yfinance 限制資料範圍，樣本數可能較少。", icon="⚠️")
    ...
    if st.button(
        ...
        for _ai, _atk in enumerate(selected_tickers):
            ...
            _status_ph.info(f"⏳ **{_atk}**：下載資料並計算中…")
            ...
            if _dsig is None:
                _auto_results.append({"ticker": _atk, "status": "❌", "n_conds": 0, "msg": str(_n_or_err)})
                _status_ph.error(f"❌ **{_atk}** 回測失敗：{_n_or_err}")
                continue

            _merged = _merge_dims_to_conds(_dsig, _dvol, _dkl,
            ...
            if _merged.empty:
                ...
                _auto_results.append({"ticker": _atk, "status": "⚠️", "n_conds": 0,
            ...
            _auto_results.append({"ticker": _atk, "status": "✅",
                                   "n_conds": len(_merged),
                                   "msg": f"寫入 {len(_merged)} 條條件（{_auto_period}/{_auto_interval}）"})
        ...
        if _ok_list:
            st.success(f"🎯 成功更新 **{len(_ok_list)}** 支股票的 Telegram 條件表！", icon="🎯")
            st.balloons()
    ...
    with tabs[tab_idx]:
        ...
        with _sw_info:
            if st.session_state[_tg_en_key]:
                ...
                st.warning(f"🔴 **{ticker} Telegram 已關閉（調參模式）**", icon="🔕")
        ...
        try:
            ...
            with _col_info:
                if st.session_state["tg_match_mode"] == "all":
                    st.caption("🔁 **全部比對模式**：每一個符合的條件都各自發一條 Telegram 訊息。")
                else:
                    st.caption("1️⃣ **第一個匹配模式**：找到第一個符合的條件就觸發並停止。")
            ...
            # ── BUG FIX 1: _build_tg_msg now receives actual arguments ────────
            def _build_tg_msg(rank: str, backtest_wr: str, match_no: int,
                              total_matches: int, direction: str = "做多") -> str:
                _header = (
                    f"{'='*28}\n"
                    f"🚨 Telegram 觸發條件匹配"
            ...
            if _total_matched == 0:
                ...
                if not _tg_on:
                    ...
                    # BUG FIX 2: pass actual arguments to _build_tg_msg
                    #            and fix indentation of if _ok: block
                    for _mn, (_rank, _wr, _ci, _dir) in enumerate(_matched_list, start=1):
                        ...
                        if _tg_already_sent(_dedup_key, TG_COOLDOWN):
                            ...
                            continue
                        _msg = _build_tg_msg(_rank, _wr, _mn, _total_matched, _dir)
                        _ok, _err = send_telegram_alert(_msg)
                        if _ok:
                            _send_ok_count += 1
                            st.toast(
                    ...
                    if _send_ok_count > 0 and not _send_err_msgs:
                        ...
                        if not (BOT_TOKEN and CHAT_ID):
                            st.warning(
                                f"⚠️ 條件已匹配（{_total_matched} 條）但 **Telegram 未設定**。\n\n"
            ...
            # ── BUG FIX 3 & 4: Breakout / Breakdown alerts ────────────────────
            # Combined _tg_on + _tg_already_sent check to avoid double-send
            ...
            if pd.notna(data["High_Max"].iloc[-1]) and data["High"].iloc[-1] >= data["High_Max"].iloc[-1]:
                ...
                if _tg_on_bo and not _tg_already_sent(_bo_key, TG_COOLDOWN):
            ...
            if pd.notna(data["Low_Min"].iloc[-1]) and data["Low"].iloc[-1] <= data["Low_Min"].iloc[-1]:
                ...
                # BUG FIX 4: add _tg_already_sent dedup for breakdown too
                _bd_key = f"{ticker}_breakdown_{datetime.now().strftime('%Y%m%d_%H')}"
                if _tg_on_bo and not _tg_already_sent(_bd_key, TG_COOLDOWN):
            ...
            with st.expander(f"🔬 {ticker} 回測分析（點擊展開）", expanded=False):
                ...
                _INTERVAL_PERIOD_MAP = {
                    ...
                    "1d":  {"periods": ["3mo","6mo","1y","2y","5y","ytd","max"], "default": "2y", "help": "日線建議 2y 以上"},
                    "1wk": {"periods": ["6mo","1y","2y","5y","ytd","max"], "default": "5y", "help": "週線建議 5y 以上"},
                    "1mo": {"periods": ["1y","2y","5y","ytd","max"], "default": "max", "help": "月線建議 max"},
                }
                _all_intervals = ["1m","5m","15m","30m","1h","1d","1wk","1mo"]

                st.caption(f"ℹ️ 主監控目前：**{selected_period}** / **{selected_interval}**")

                _col_i, _col_p = st.columns(2)
                bt_interval = _col_i.selectbox(
                    "回測K線間隔", _all_intervals, index=_all_intervals.index("1d"),
                ...
                _BARS_PER_DAY = {"1m":390, "5m":78, "15m":26, "30m":13, "1h":7, "1d":1, "1wk":1, "1mo":1}
                _PERIOD_DAYS  = {"1d":1, "5d":5, "7d":7, "60d":60, "1mo":21, "3mo":63,
                ...
                st.caption(f"{_est_icon} 預估約 **{_est_n}** 根K線")
                ...
                bt_pnl_thr = col_e.number_input("最低平均盈虧 (%)", -10.0, 20.0, 0.0, 0.1,
                ...
                if st.button("🚀 開始回測", type="primary", key=f"bt_run_{ticker}"):
                    with st.spinner(f"正在計算 {ticker}（{bt_period} / {bt_interval}）三維勝率…"):
                        try:
                            ...
                            bt_data["Datetime"] = pd.to_datetime(bt_data["Datetime"]).dt.tz_localize(None)
                            if len(bt_data) < 30:
                                st.warning(f"資料不足（{len(bt_data)} 根K線 < 30）。")
                                st.stop()
                ...
                if f"bt_df_sig_{ticker}" in st.session_state:
                    ...
                    st.info(
                        f"📊 本次回測：**{_bt_lbl}**　範圍：**{_bt_period_used}**　"
                        f"間隔：**{_bt_interval_used}**　共 **{_bt_bars_used}** 根K線"
                        + ("　⚠️ 樣本偏少" if isinstance(_bt_bars_used, int) and _bt_bars_used < 100 else "")
                    )
                    ...
                    def _render_dim(df_dim: pd.DataFrame, title: str, wr_thr: int,
                        ...
                        if not hi.empty:
                            ...
                            st.dataframe(
                                hi[disp_cols].style.background_gradient(
                                    subset=["勝率(%)"], cmap="Greens",
                                    gmap=hi["勝率(%)"].apply(pd.to_numeric, errors="coerce")),
                                use_container_width=True,
                                ...
                                if st.button(f"📊 展開逐筆交易記錄", key=f"detail_btn_{dim_key}_{ticker}"):
                                    ...
                                    if _bt_raw is None:
                                        ...
                                        if _detail.empty:
                                            ...
                                            _fig_pnl.add_hline(y=0, line_dash="dash", line_color="gray", line_width=0.8)
                                            _fig_pnl.update_layout(
                                                title="累計盈虧曲線",
                                                xaxis_title="交易序號", yaxis_title="累計盈虧 (%)",
                                            ...
                                            _fig_bar = go.Figure(go.Bar(
                                                x=_detail["序號"], y=_detail["盈虧(%)"],
                                                marker_color=["#2ecc71" if v >= 0 else "#e74c3c" for v in _detail["盈虧(%)"]],
                                                name="單筆盈虧",
                                            ))
                                            _fig_bar.add_hline(y=0, line_dash="dash", line_color="gray", line_width=0.8)
                                            _fig_bar.update_layout(
                                                title="逐筆盈虧分佈",
                                                xaxis_title="交易序號", yaxis_title="盈虧 (%)",
                                            ...
                                            _disp_detail = _detail[[
                                                ...
                                                "進場價","出場價","方向","盈虧(%)","勝負",
                                                "最大順勢(%)","最大逆勢(%)","累計盈虧(%)",
                                                "進場RSI","進場MACD","成交量標記","K線形態",
                                                "連勝數","連敗數","觸發信號",
                                            ]]
                                            try:
                                                def _row_color(row):
                                                    is_win = row["勝負"] == "✅ 勝"
                                                    c = ("background-color: rgba(46,204,113,0.15)"
                                                         if is_win else
                                                         "background-color: rgba(231,76,60,0.15)")
                                                    return [c] * len(row)
                                                st.dataframe(
                                                    _disp_detail.style.apply(_row_color, axis=1),
                                                    use_container_width=True,
                        ...
                        with st.expander(f"📊 {title} 全部 {total} 組（展開查看）"):
                            ...
                            st.dataframe(
                                df_dim[disp_all].style.background_gradient(
                                    subset=["勝率(%)"], cmap="RdYlGn",
                                    gmap=df_dim["勝率(%)"].apply(pd.to_numeric, errors="coerce")),
                    ...
                    def _one_click_add(hi_df: pd.DataFrame, dim_key: str):
                        ...
                        if "方向" not in combined.columns:
                            combined["方向"] = ""
                        _oc_saved = combined[["排名","異動標記","成交量標記","K線形態","回測勝率","方向"]]
                    ...
                    if all_hi:
                        ...
                        st.info(f"三個維度均無 ≥ {_wr_thr}% 勝率組合。建議延長時間範圍或降低閾值。")
                    ...
                    _merge_thr = _merge_col1.number_input(
                        "納入勝率閾值 (%)", min_value=0, max_value=100,
                        value=_wr_thr, step=5, key=f"merge_thr_{ticker}",
                    )
                    ...
                    def _preview_merge(thr: float):
                        ...
                        for _, r in df_sig.iterrows():
                            if r["勝率(%)"] >= thr:
                                rows.append({"異動標記": r["信號組合"].replace(" + ", ", "),
                                             "成交量標記":"—","K線形態":"—",
                                             "回測勝率":f"{r['勝率(%)']:.1f}%",
                                             "方向":r.get("方向","做多"),"_wr_num":r["勝率(%)"]})
                        for _, r in df_vol.iterrows():
                            if r["勝率(%)"] >= thr:
                                rows.append({"異動標記": r["信號組合"].replace(" + ", ", "),
                                             "成交量標記":r.get("成交量標記","—"),"K線形態":"—",
                                             "回測勝率":f"{r['勝率(%)']:.1f}%",
                                             "方向":r.get("方向","做多"),"_wr_num":r["勝率(%)"]})
                        for _, r in df_kl.iterrows():
                            if r["勝率(%)"] >= thr:
                                rows.append({"異動標記": r["信號組合"].replace(" + ", ", "),
                                             "成交量標記":"—","K線形態":r.get("K線形態","—"),
                                             "回測勝率":f"{r['勝率(%)']:.1f}%",
                                             "方向":r.get("方向","做多"),"_wr_num":r["勝率(%)"]})
                        if not rows:
                            return pd.DataFrame()
                    ...
                    with _merge_col3:
                        if _n_preview == 0:
                            st.warning(f"無勝率 ≥ {_merge_thr}% 的組合，請降低閾值或重新回測。")
                        else:
                            st.info(f"✅ 合併後共 **{_n_preview}** 條，將覆蓋現有條件表。")

                    if not _preview_df.empty:
                        with st.expander(f"👁️ 預覽合併結果（{_n_preview} 條）"):
                            st.dataframe(_preview_df, use_container_width=True,
                                         height=min(500, 38 * (_n_preview + 1) + 40))

                    if st.button(
                        f"🔀 確認合併並覆蓋 Telegram 條件表（共 {_n_preview} 條）",
                        type="primary", disabled=(_n_preview == 0),
                        key=f"merge_all_dims_btn_{ticker}",
                    ):
                        st.session_state[_ss_key(ticker)] = _preview_df.copy()
                        _tg_save(_preview_df.copy(), ticker)
                        st.success(
                            f"🎯 **覆蓋完成！** 共 **{_n_preview}** 條條件（勝率 ≥ {_merge_thr}%）。"
                        )
                        st.balloons()
